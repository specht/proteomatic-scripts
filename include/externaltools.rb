...
class ExternalTools
	...
	def self.binaryPath(as_Tool)
		lk_ToolDescription = YAML::load_file(File::join(@@ms_RootPath, "include/properties/ext/ext.#{as_Tool}.yaml"))
		ls_Package = as_Tool.split('.').first
		lk_PackageDescription = YAML::load_file(File::join(@@ms_RootPath, "include/properties/ext/ext.#{ls_Package}.yaml"))
	...
	def self.installed?(as_Package)
		lb_Ok = true
		lk_PackageDescription = YAML::load_file(File::join(@@ms_RootPath, "include/properties/ext/#{as_Package}.yaml"))
		ls_Package = as_Package.sub('ext.', '')
		begin
