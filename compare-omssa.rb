    ...
    def run()
        ...
        if @output[:htmlReport]
            File.open(@output[:htmlReport], 'w') do |lk_Out|
                ...
                if @param[:printPeptidesToHtml]
                    lk_Out.puts 'tr.thickborder { background-color: #eee; }'
                end
                lk_Out.puts '.default { }'
                ...
                lk_Out.puts '<table>'
                lk_Out.puts '<thead>'
                lk_Out.puts "<tr><th rowspan='2'>Protein</th><th colspan='#{lk_RunKeys.size}'>Spectra count</th><th rowspan='2'>SD</th><th colspan='#{lk_RunKeys.size}'>Distinct peptide count</th><th rowspan='2'>SD</th></tr>"
                ...
                lk_Proteins.each do |ls_Protein|
                    ...
                    # print protein
                    lk_Out.print "<tr class='thickborder'><td>#{ls_Protein}</td>"
                    ...
                    if @param[:printPeptidesToHtml]
                        ...
                        thisPeptides.each do |ls_Peptide|
                            lk_Out.print "<tr style='color: #666;'><td>#{ls_Peptide}</td>"
                            ...
                            lk_Out.print ls_SpectralCountString
                            lk_Out.print "<td>#{sprintf("%1.2f", stddev(lk_Values))}</td>"
                            lk_Out.print "<td colspan='#{lk_RunKeys.size + 1}'></td>"
